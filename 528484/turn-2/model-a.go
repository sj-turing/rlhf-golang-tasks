...
func (kv *KeyValueStore) LinkUserToOrder(userID, orderID int64) {
	kv.mu.Lock()
	kv.userOrders[userID] = append(kv.userOrders[userID], orderID)
	kv.mu.Unlock()
}
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				return
			default:
				orderInfoChan <- Order{ID: time.Now().UnixMilli(), Description: randomString(rand.Intn(80)), Amount: rand.Float64() * 100}
			}
		}
	}()
...
func main() {
	...
	for size > 0 {
		...
		kvStore.GetUser(key)
		kvStore.GetOrder(key)
		size--
	}

	wg.Wait()

	fmt.Println("records created: ", len(kvStore.users), len(kvStore.orders))
}
