...
func (s *UserService) GetUsersByAddressOptimal(w http.ResponseWriter, r *http.Request) {
	log.Println("Inside GetUsersByAddressOptimal")
...
func (s *UserService) GetUsersByAddress(w http.ResponseWriter, r *http.Request) {
	log.Println("Inside GetUsersByAddress")
	address := r.URL.Query().Get("address")
	if address == "" {
...
func main() {
	...
	http.HandleFunc("/getusersbyaddressnew", userService.GetUsersByAddressOptimal)
