...
// UserAddressIndex is a map that indexes users by their address
type UserAddressIndex struct {
	mu    sync.RWMutex
	index map[string][]*User
}

func NewUserAddressIndex() *UserAddressIndex {
	return &UserAddressIndex{
		index: make(map[string][]*User),
	}
}

// UpdateUserAddressIndex updates the user address index after user update
func (s *UserService) UpdateUserAddressIndex(user *User, oldAddress string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	// Remove user from old address index
	for i, u := range s.index[oldAddress] {
		if u.Username == user.Username {
			s.index[oldAddress] = append(s.index[oldAddress][:i], s.index[oldAddress][i+1:]...)
			break
		}
	}
	// Add user to the new address index
	s.index[user.Address] = append(s.index[user.Address], user)
}

// GetUsersByAddress retrieves users based on the given address using the index
func (s *UserService) GetUsersByAddress(w http.ResponseWriter, r *http.Request) {
	address := r.URL.Query().Get("address")
	if address == "" {
		respondWithJSON(w, http.StatusBadRequest, APIResponse{Status: "error", Message: "Address query param is required"})
